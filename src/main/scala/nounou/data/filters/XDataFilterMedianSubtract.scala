...
//      val tempData = upstreamBuff.readTrace( channel, FrRange(frame - windowLengthHalf, frame + windowLengthHalf, 1))//, OptSegment(segment)) )
...
//    } else {
//      //by calling _parent.readTrace instead of _parent.readTraceImpl, we can deal with cases where the kernel will overhang actual data, since the method will return zeros
//      val tempData = upstreamBuff.readTrace( channel, FrRange( ran.start - windowLengthHalf, ran.last + windowLengthHalf, 1))//, OptSegment(segment)) )
