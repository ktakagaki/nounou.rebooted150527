...
import nounous.io.LittleEndianByteConverter._
import org.scalatest.FunSuite

/**
 * Created by Kenta on 12/10/13.
 */
class LittleEndianByteConverter$Test extends FunSuite {
...
//    assert(valueS == bytesToInt16(ba(0), ba(1)))
  ...
  test("UInt16"){
    var valueS: Int = 0
    var ba = uInt16ToBytes( valueS )
    assert(ba.length == 2)
    assert(valueS == bytesToUInt16(ba(0), ba(1)))

    valueS = 30000
    ba = uInt16ToBytes( valueS )
    assert(valueS == bytesToUInt16(ba(0), ba(1)))
